# FFA-X logistics cells

The Karrio open-source server is not used as a shared FFA-X multi-tenant database.
Each cell has an independent PostgreSQL database, Redis volume, API secret and
carrier plugin directory. FFA-X assigns one tenant to a cell and the logistics
...
this directory, Compose files, Git, browser storage or FFA-X connector settings.
